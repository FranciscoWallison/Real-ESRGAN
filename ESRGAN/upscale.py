    ...
    def run(self) -> None:
        ...
        with Progress(
            ...
            for idx, img_path in enumerate(images, 1):
                ...
                # print("str(img_output_path_rel.absolute())_new ", str(img_output_path_rel.absolute()))
                # print("str(img_path.absolute())_ould ", str(img_path.absolute()))
                # # Abrir a imagem
                # img_oud = cv2.imread(str(img_path.absolute()))
                # img_new = cv2.imread(str(img_output_path_rel.absolute()))

                # # Obter a largura e a altura da imagem
                # altura, largura, canais = img_oud.shape
                # print("largura, altura ", largura, altura)

                # altura_new, largura_new, canais_new = img_new.shape
                # print("altura_new, largura_new ", altura_new, largura_new)

                # nova_imagem = cv2.resize(img_new, (largura, altura))
                # cv2.imwrite(str(img_output_path_rel.absolute()), nova_imagem)
